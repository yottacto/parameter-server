...
class ParameterServer(object):
    def __init__(self, keys, values, bounded_delay=3):
    ...
    def blocked(self, worker_iter):
        straggler_iter = min(self.worker_iter.values()) if self.worker_iter else 0
        return straggler_iter + self.bounded_delay < worker_iter

    def push(self, keys, values, worker_id, worker_iter):
        # assume worker_iter monotonically increase
        self.worker_iter[worker_id] = worker_iter
