...
def accuracy_from_confusion(confusion_matrix):
   return sum(confusion_matrix[i][i] for i in range(confusion_matrix.shape[0])) / sum(map(sum, confusion_matrix))
...
class simple(object):
    ...
    def compute_confusion(self, x, y):
        return self.sess.run(self.confusion,
                             feed_dict={self.x: x, self.y: y})
    ...
    def __init__(self, learning_rate=1e-4):
        with tf.Graph().as_default():
            ...
            with tf.name_scope('confusion'):
                ...
                predictions = tf.argmax(self.out_layer, 1)
